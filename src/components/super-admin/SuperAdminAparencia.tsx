...
export function SuperAdminAparencia() {
  ...
  const [color, setColor] = useState(systemColor);
  ...
  }, [initialized, isLoading, systemColor]);

  const updateColor = useMutation({
    mutationFn: async (nextColor: string) => {
      const { error } = await supabase.rpc(
        "sa_update_platform_settings" as never,
        {
          _primary_color: nextColor,
        } as never,
      );
    ...
    onError: () => {
      toast.error("Erro ao salvar cor do sistema.");
    },
  });
