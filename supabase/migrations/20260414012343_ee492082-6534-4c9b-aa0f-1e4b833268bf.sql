...
BEGIN
  ...
  -- por_unidade requires unidade_cobranca
  IF NEW.modo_cobranca = 'por_unidade' AND (NEW.unidade_cobranca IS NULL OR btrim(NEW.unidade_cobranca) = '') THEN
    RAISE EXCEPTION 'modo_cobranca por_unidade exige unidade_cobranca preenchida';
  END IF;
